...
func (_Groups) NewSingleton(otx *sql.Tx, uid UserID) (GroupID, error) {
	...
	q := `
	INSERT INTO wf_groups(name, group_type)
	SELECT u.email, 'S'
	FROM wf_users u
	WHERE u.id = ?
	`
...
func (_Groups) New(otx *sql.Tx, name string, gtype string) (GroupID, error) {
	...
	res, err := tx.Exec("INSERT INTO wf_groups(name, group_type) VALUES(?, ?)", name, gtype)
	if err != nil {
		return 0, err
	}
	var id int64
...
func (_Groups) Get(id GroupID) (*Group, error) {
	...
	var elem Group
	row := db.QueryRow("SELECT id, name, group_type FROM wf_groups WHERE id = ?", id)
...
func (_Groups) Rename(otx *sql.Tx, id GroupID, name string) error {
	...
	_, err = tx.Exec("UPDATE wf_groups SET name = ? WHERE id = ?", name, id)
	if err != nil {
		return err
	}
...
func (_Groups) Delete(otx *sql.Tx, id GroupID) error {
	...
	row := db.QueryRow("SELECT group_type FROM wf_groups WHERE id = ?", id)
	var gtype string
	err := row.Scan(&gtype)
	if err != nil {
		return err
	...
	res, err := tx.Exec("DELETE FROM wf_groups WHERE id = ?", id)
	if err != nil {
		return err
	}
...
func (_Groups) SingletonUser(gid GroupID) (*User, error) {
	...
	JOIN wf_groups gm ON gus.group_id = gm.id
	WHERE gm.id = ?
	AND gm.group_type = 'S'
	ORDER BY um.id
	LIMIT 1
...
func (_Groups) AddUser(otx *sql.Tx, gid GroupID, uid UserID) error {
	...
	var gtype string
	row := tx.QueryRow("SELECT group_type FROM wf_groups WHERE id = ?", gid)
	err := row.Scan(&gtype)
	if err != nil {
		return err
	}
